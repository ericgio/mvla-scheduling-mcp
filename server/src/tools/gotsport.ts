...
const gotsportSchema = {
  event_id: z
    ...
    .describe(
      'GotSport event (season) ID — from the URL: /org_event/events/<ID>/',
    ),
  team_id: z
    .union([z.string(), z.number()])
    .describe(
      'GotSport team registration ID — from the URL query string: ?team=<ID>',
    ),
  force_refresh: z
    .boolean()
    .optional()
    ...
    async ({ event_id, team_id, force_refresh }) =>
      withToolLogging(
        ...
        async () => {
          ...
          const url = `https://system.gotsport.com/org_event/events/${event_id}/matches_export?team=${team_id}`;

          const { bytes, contentType, contentDisposition } = await fetchBytes(
            url,
            {
              method: 'POST',
              headers: {
          ...
          if (
            ...
            bytes.subarray(0, 5).toString() === '<!DOC'
          ) {
            throw new Error(
